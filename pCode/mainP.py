...
def create_client(client: ClientCreate, db: Session = db_dependency):
    ...
    if existing_client:
     ...
     # Auto-assign trainer based on client goal
    assigned_trainer = db.query(Trainer).filter(Trainer.specialization == client.client_goal).first()
    if not assigned_trainer:
        raise HTTPException(status_code=404, detail="No trainer available for this goal")

    # Assign the matched trainer ID
    db_client = Client(
        ...
        client_goal=client.client_goal,
        client_trainer_id=assigned_trainer.trainer_id,  # Auto-assigned trainer
    )
    ...
    db_client = Client(
        ...
        client_goal = models.WorkoutPlanEnum
    )
    db.add(db_client)
    db.commit()
